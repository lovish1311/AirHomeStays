...
class FragmentReviewByYouPast : BaseFragment<FragmentReviewBinding, ReviewViewModel>(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        pagingController = ReviewListController(this.requireContext(),"byYou",viewModel)
        CustomSpringAnimation.spring(mBinding.rvReviewList)
        subscribeToLiveData()
        viewModel.netWorkStateByYou.observe(viewLifecycleOwner, Observer {
            it?.let { networkState ->
                when (networkState) {
    ...
    fun subscribeToLiveData(){
        viewModel.loadByYouList().observe(viewLifecycleOwner, Observer<PagedList<GetUserReviewsQuery.Result>>{
            it?.let {
