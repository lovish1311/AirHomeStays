        ...
        fun bind(item: GetThreadsQuery.ThreadItem) {
            if (item.startDate != null && item.endDate != null) {
                with(binding) {
                    ...
                    val guest = if (item.personCapacity!=0) item.personCapacity.toString()+" Guests " else ""
                    val pet = if (item.pets!=0) ","+item.pets.toString()+" pets " else ""
                    val infant = if (item.infants!=0) ","+item.infants.toString()+" infants " else ""
                    val visitors = if (item.visitors!=0) ","+item.visitors.toString()+"  visitors" else ""
                    noOfExtra = guest + pet + infant + visitors
                }
            }
            item.type?.let {
